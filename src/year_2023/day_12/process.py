...
def arrangements(condition_record: str, damaged_springs: tuple[int, ...]) -> int:
    ...
    if end_of_condition_record:
        ...
        pattern = (
            f'{MAYBE_DAMAGED}{{{damaged_springs[0]}}}'
            f'{MAYBE_OPERATIONAL}*'
        )
        ...
        if re.match(pattern, condition_record):
            option_1 = arrangements(condition_record[damaged_springs[0] + 1:], damaged_springs[1:])
...
def main() -> None:
    ...
    print(
        "Sum of all possible spring arrangements:",
        sum(folded_field.arrangements())
    )
