...
class SmokeBasin:
    def __init__(self, heightmap):
        self.heightmap = [
            [int(point) for point in row] for row in heightmap.split("\n")
        ]
    ...
    def _get_low_points(self):
        ...
        for y, row in enumerate(self.heightmap):
            for x, point in enumerate(row):
                ...
                neighbours = [
                    self.heightmap[coord.y][coord.x] for coord in neighbour_coords
                ]
    ...
    def calculate_low_point_risk_level(self):
        ...
        low_point_values = [
            self.heightmap[coord.y][coord.x] for coord in low_point_coords
        ]
    ...
    def _get_basin_size(self, low_point):
        ...
        while coords_queue:
            ...
            for neighbour_coord in neighbour_coords:
                if (
                    neighbour_coord not in seen
                    and self.heightmap[neighbour_coord.y][neighbour_coord.x] != 9
                ):
    ...
    def three_largest_basins_size(self):
        low_point_coords = self._get_low_points()
        basin_sizes = [
            self._get_basin_size(low_point) for low_point in low_point_coords
        ]
