...
from advent_of_code.puzzles.year_2021.day_14 import process
...
CN -> C"""
    ...
    step_1 = "NCNBCHB"
    assert next(ep) == collections.Counter(
        "".join(pair) for pair in more_itertools.pairwise(step_1)
    )
    assert ep.polymer_length() == len(step_1)

    step_2 = "NBCCNBBBCBHCB"
    assert next(ep) == collections.Counter(
        "".join(pair) for pair in more_itertools.pairwise(step_2)
    )
    assert ep.polymer_length() == len(step_2)

    step_3 = "NBBBCNCCNBBNBNBBCHBHHBCHB"
    assert next(ep) == collections.Counter(
        "".join(pair) for pair in more_itertools.pairwise(step_3)
    )
    assert ep.polymer_length() == len(step_3)

    step_4 = "NBBNBNBBCCNBCNCCNBBNBBNBBBNBBNBBCBHCBHHNHCBBCBHCB"
    assert next(ep) == collections.Counter(
        "".join(pair) for pair in more_itertools.pairwise(step_4)
    )
