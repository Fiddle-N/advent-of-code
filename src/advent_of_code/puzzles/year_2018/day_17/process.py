...
class ReservoirResearch:
    ...
    def _clay_calc(self, input_):
        ...
        for vein in input_.split("\n"):
            ...
            pos_2_start, pos_2_end = [
                int(pos_val) for pos_val in raw_pos_2_range.split("..")
            ]
    ...
    def _water_handler(self, water, water_state):
        while True:  # while water.y <= self.max_y or while True?
            ...
            if next_water_state == WaterState.WATER_FLOWING_FROM_OTHER_SOURCE:
                ...
                water_state == WaterState.WATER_FLOWING
                and next_water_state == WaterState.WATER_FLOWING
            ):
                ...
                and next_water_state == WaterState.WATER_SETTLED
            ):
                next_water_y = water.y - 1
                water = Coords(water.x, next_water_y)
    ...
    def _water_flowing(self, water, water_state):
        ...
        if (
            ground_below_water in self.water
            and self.water[ground_below_water] == Ground.WATER_FLOWING
        ):
    ...
    def _water_on_top_of_clay(self, water):
        ...
        left_water_coords, left_water_state, left_last_water = (
            self._water_on_top_of_clay_one_side(water, Side.LEFT)
        )
        right_water_coords, right_water_state, right_last_water = (
            self._water_on_top_of_clay_one_side(water, Side.RIGHT)
        )
        ...
        if all(
            ...
            for water_state in (left_water_state, right_water_state)
        ):
    ...
    def _water_on_top_of_clay_one_side(self, water, side: Side):
        ...
        while boundary_comp(x, x_bound):
            ...
            water_coords.append(
                next_coords
            )  # this coordinate must be reachable by water (either settled or not)
            beneath_next_coords = Coords(x, water.y + 1)
            if beneath_next_coords in self.clay or (
                beneath_next_coords in self.water
                and self.water[beneath_next_coords] == Ground.WATER_SETTLED
            ):
                x = side_op(x, 1)
            else:
                water_state = WaterState.WATER_FLOWING
