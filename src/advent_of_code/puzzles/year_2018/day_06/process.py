...
class ChronalCoods:
    ...
    def calculate_nearest_coords(self):
        for x, y in itertools.product(
            range(self.min_x, self.max_x + 1), range(self.min_y, self.max_y + 1)
        ):
            distances = [
                abs(coord_x - x) + abs(coord_y - y) for coord_x, coord_y in self.coords
            ]
            sorted_distances = sorted(distances)
            if (
                not sorted_distances[0] == sorted_distances[1]
            ):  # not equally close to at least two coords
                nearest_coord = distances.index(
                    sorted_distances[0]
                )  # retrieve closest coord
                self.areas[nearest_coord] += 1
                if x in (self.min_x, self.max_x) or y in (
                    self.min_y,
                    self.max_y,
                ):  # at edge of grid
    ...
    def retrieve_largest_finite_area(self):
        finite_areas = [
            ...
            if coord not in self.infinite_coords
        ]
