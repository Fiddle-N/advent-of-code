...
def _is_valid_number_between(year, start, end, allow_leading_zeros=False):
    return _is_valid_number(year, allow_leading_zeros) and _is_number_between(
        year, start, end
    )
...
def is_valid_height(height):
    if (height_re := re.fullmatch(r"(\d{2,3})(cm|in)", height)) is None:
        return False
...
def is_valid_passport_id(passport_id):
    return len(passport_id) == 9 and _is_valid_number(
        passport_id, allow_leading_zeros=True
    )
