...
class AllergenAssessment:
    def __init__(self, foods):
        self.foods = foods
        ingredients = itertools.chain.from_iterable(
            food.ingredients for food in self.foods
        )
    ...
    def from_string(cls, food_str):
        ...
        for line in food_str.splitlines():
            if (line_re := re.match(r"(.*) \(contains (.*)\)", line)) is not None:
    ...
    def _extract_allergenic_ingredients(self):
        ...
        for allergen in self.allergens:
            foods_containing_allergen = [
                food for food in self.foods if allergen in food.allergens
            ]
            ...
            if possible_ingredients_for_allergen:
                possible_allergenic_ingredients[allergen] = (
                    possible_ingredients_for_allergen
                )
    ...
    def _resolve_allergenic_ingredients(possible_allergenic_ingredients):
        while not all(
            (len(ingredients) == 1)
            for ingredients in possible_allergenic_ingredients.values()
        ):
            unresolved_allergenic_ingredients = possible_allergenic_ingredients
            resolved_ingredients = set.union(
                *[
                    ingredients
                    for ingredients in unresolved_allergenic_ingredients.values()
    ...
    def _calculate_allergenic_ingredients(self):
        ...
        allergenic_ingredients = self._resolve_allergenic_ingredients(
            possible_allergenic_ingredients
        )
        return allergenic_ingredients
    ...
    def canonical_dangerous_ingredients(self):
        return ",".join(
            ingredients
            for _, ingredients in sorted(
                self.allergenic_ingredients.items(), key=lambda x: x[0]
            )
        )
...
def main():
    ...
    print(
        f"Non allergenic ingredient occurrence: {allergen_assessment.non_allergenic_ingredient_occurrence()}"
    )
    print(
        f"Canonical dangerous ingredients: {allergen_assessment.canonical_dangerous_ingredients}"
    )
