...
class AdapterArray:
    def __init__(self, joltage_input=None):
        joltage_input = (
            joltage_input if joltage_input is not None else self._read_file()
        )
    ...
    def jolts(self):
        ...
        for lower_jolt, higher_jolt in more_itertools.windowed(self.joltage, 2):
            diff = higher_jolt - lower_jolt  # type:ignore[unsupported-operator]
            if diff == 1:
                one_jolts += 1
            elif diff == 3:
                three_jolts += 1
    ...
    def distinct_arrangements(self, style="calculated"):
        one_jolt_differences = more_itertools.split_when(
            self.joltage, lambda x, y: y - x == 3
        )
        one_jolt_difference_sections_greater_than_2 = [
            section for section in one_jolt_differences if len(section) > 2
        ]
        section_lengths = [
            len(section) for section in one_jolt_difference_sections_greater_than_2
        ]
        if style == "calculated":
            length_to_arrangement = {
                length: self._arrangements(length) for length in set(section_lengths)
            }
            ...
            raise Exception
        arrangements = [
            length_to_arrangement[section_length] for section_length in section_lengths
        ]
...
def main():
    ...
    print(
        f"Adapter distinct arrangements: {adapter_array.distinct_arrangements(style='hardcoded')}"
    )
