...
def test_hand_comparisons_with_different_types():
    ...
    for stronger_hand_input, weaker_hand_input in itertools.pairwise(hands):
        stronger_hand = process.Hand.from_string(stronger_hand_input[0])
        assert stronger_hand.type() == stronger_hand_input[1]

        weaker_hand = process.Hand.from_string(weaker_hand_input[0])
...
def test_hand_comparison_with_same_types(stronger_hand_input, weaker_hand_input, hand_type):
    stronger_hand = process.Hand.from_string(stronger_hand_input)
    weaker_hand = process.Hand.from_string(weaker_hand_input)
    assert stronger_hand.type() == weaker_hand.type() == hand_type
    assert stronger_hand > weaker_hand


def test_hand_type_with_joker_rule():
    hand = process.Hand.from_string('QJJQ2', joker_rule=True)
    assert hand.type() == process.Type.FOUR_OF_A_KIND


def test_all_joker_hand_type():
    hand = process.Hand.from_string('JJJJJ', joker_rule=True)
...
QQQJA 483"""
    ...
    ranked = game.ranked()
    assert ranked == {
        1: process.HandBid(process.Hand.from_string('32T3K'), 765),
        2: process.HandBid(process.Hand.from_string('KTJJT'), 220),
        3: process.HandBid(process.Hand.from_string('KK677'), 28),
        4: process.HandBid(process.Hand.from_string('T55J5'), 684),
        5: process.HandBid(process.Hand.from_string('QQQJA'), 483),
    }
...
QQQJA 483"""
    ...
    ranked = game.ranked()
    assert ranked == {
        1: process.HandBid(process.Hand.from_string('32T3K', joker_rule=True), 765),
        2: process.HandBid(process.Hand.from_string('KK677', joker_rule=True), 28),
        3: process.HandBid(process.Hand.from_string('T55J5', joker_rule=True), 684),
        4: process.HandBid(process.Hand.from_string('QQQJA', joker_rule=True), 483),
        5: process.HandBid(process.Hand.from_string('KTJJT', joker_rule=True), 220),
    }
