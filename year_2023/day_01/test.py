...
def test_get_cal_vals_without_word_numbers():
    assert (
        process.get_cal_vals("""\
1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet""".splitlines(), include_word_numbers=False)
        == [12, 38, 15, 77]
    )
...
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet""".splitlines(), include_word_numbers=False) == 142
...
4nineeightseven2
zoneight234
7pqrstsixteen""".splitlines(), include_word_numbers=True)
...
4nineeightseven2
zoneight234
7pqrstsixteen""".splitlines(), include_word_numbers=True) == 281
...
eightoneight""".splitlines(), include_word_numbers=True) == [88]
